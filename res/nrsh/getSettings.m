function info = getSettings(varargin)
    ...
    %       method@char-array...    {'asm', 'fresnel', 'fourier-fresnel'} or any of it appended by
    %       zero_pad@boolean(1)
    %       apod@boolean(1)
    %       perc_clip@boolean(1)
    ...
    %
    % To reconstruct without a configuration file, the following options are required:
    %   wlen, pixel_pitch, method, evtl. usage_mode, h_pos, v_pos, ap_sizes, ref_wave_rad
    %
    %-------------------------------------------------------------------------
    ...
    % Valid field names
    validFieldnameList = {'orthographic', 'verbosity', 'usagemode', 'apertureinpxmode', 'ap_sizes', 'h_pos', 'v_pos', ...
