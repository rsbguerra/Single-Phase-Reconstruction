...
namespace {
    ...
    template<typename T> T* toAttributeImp(const mxArray* pa) {
        mexWarnMsgIdAndTxt(
            "OpenEXR:unsupported",
            "Unsupported conversion to Imf::Attribute from \"%s\"",
            mxGetClassName(pa));
        return NULL;
    }

    template<> StringAttribute* toAttributeImp(const mxArray* pa) {
        std::string value;
    ...
    template<> StringVectorAttribute* toAttributeImp(const mxArray* pa) {
        StringVector value;
    ...
    template<> IntAttribute* toAttributeImp(const mxArray* pa) {
        int value = 0;
    ...
    template<> FloatAttribute* toAttributeImp(const mxArray* pa) {
        float value = 0.0f;
        OpenEXRforMatlab::convertData(&value, pa, mxGetClassID(pa), 1);
    ...
    template<> DoubleAttribute* toAttributeImp(const mxArray* pa) {
        double value = 0.0;
        OpenEXRforMatlab::convertData(&value, pa, mxGetClassID(pa), 1);
...
Attribute* OpenEXRforMatlab::toAttribute(const mxArray* pa) {
    if (mxIsChar(pa)) {
        ...
        if (isVector(pa, numel)) {
            ...
            for (mwIndex i = 0; i != numel; ++i) {
                if (!mxIsChar(mxGetCell(pa, i))) { return NULL; }
            }
            return toAttributeImp<StringVectorAttribute>(pa);
        }
    } else if (mxIsNumeric(pa)) {
        mwSize M = 0, N = 0;
        if (!isMatrix(pa, M, N)) { return NULL; }

        if (M == 1 && N == 1) {
            const mxClassID type = mxGetClassID(pa);
            switch (type) {
                case mxSINGLE_CLASS: return toAttributeImp<FloatAttribute>(pa);
                case mxINT8_CLASS:
                case mxUINT8_CLASS:
                case mxINT16_CLASS:
                case mxUINT16_CLASS:
                case mxINT32_CLASS: return toAttributeImp<IntAttribute>(pa);
                case mxUINT32_CLASS:
                case mxINT64_CLASS:
                case mxUINT64_CLASS:
                case mxDOUBLE_CLASS: return toAttributeImp<DoubleAttribute>(pa);
                default:
