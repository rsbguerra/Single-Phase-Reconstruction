...
namespace {
    ...
    void getRequestedChannels(int nrhs, const mxArray* prhs[],
        ...
        if (nrhs == 2) {
            ...
            if (mxIsCell(prhs[1])) {
                ...
                if (nDim != 2) {
                    mexErrMsgIdAndTxt("OpenEXR:argument",
                                      "Invalid cell array dimensions: %d",
                ...
                mexErrMsgIdAndTxt("OpenEXR:argument",
                                  "The second input argument is neither a cell "
                                  "array nor a string.");
            }
        } else if (nrhs > 2) {
            ...
            for (int i = 1; i < nrhs; ++i) {
                if (!mxIsChar(prhs[i])) {
                    const int   index = i + 1;
                    const char* suffix =
                        index == 2 ? "nd" : (index == 3 ? "rd" : "th");
                    mexErrMsgIdAndTxt("OpenEXR:argument",
                                      "The %d-%s argument is not a string",
                                      index, suffix);
                }
        ...
        for (mxArrayIter it = mxChannels.begin(); it != mxChannels.end();
             ++it) {
    ...
    void prepareFrameBuffer(FrameBuffer &fb, const Box2i &dataWindow,
        ...
        for (size_t i = 0; i != requestedChannels.size(); ++i) {
            ...
            fb.insert(requestedChannels[i].c_str(),
                      Slice(FLOAT, (char*)(ptr + offset),
                            sizeof(float) * xStride, sizeof(float) * yStride,
                            xSampling, ySampling));
        }
    }
    ...
    mxArray* buildMap(const std::vector<std::string> &channelNames,
        ...
        if (mexCallMATLAB(1, &mapHandle, 2, &mapArgs[0], "containers.Map") !=
            0) {
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    mxFree(inputfilePtr);
    inputfilePtr = static_cast<char*>(0);
    ...
    try {
        ...
        if (nlhs <= 1) {
            ...
            for (int i = 0; i < nlhs; ++i) { plhs[i] = mxData[i]; }
        }
    } catch (Iex::BaseExc &e) {
