...
namespace {
    ...
    struct Pair {
        ...
        bool isValid() const { return name != NULL && value != NULL; }
    };
    ...
    mxArray* getChannelNames(const ChannelList &channels) {
        ...
        for (ChannelIterator it = channels.begin(); it != channels.end();
             ++it) {
    ...
    mxArray* getAttributesMap(const Header &header) {
        ...
        for (Header::ConstIterator it = header.begin(); it != header.end();
             ++it) {
            const Attribute &attr = it.attribute();
            if (!Attribute::knownType(attr.typeName())) { continue; }

            Pair pair(it.name());
            pair.value = toMatlab(attr);
            if (pair.isValid()) { attributes.push_back(pair); }
        }
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    try {
        ...
        mxArray*    bStruct  = mxCreateStructMatrix(
            1, 1, sizeof(fields) / sizeof(const char*), &fields[0]);
