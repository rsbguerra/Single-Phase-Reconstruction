...
namespace {
    ...
    void readPixels(float* buffer, InputFile &file) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            framebuffer.insert(
                channels[i],
                Slice(FLOAT, (char*)(data[i] + offset), sizeof(float) * xStride,
    ...
    mxArray* readPixels(const char* filename) {
        // Check if it is a YC file, so that we use the RGBA interface to decode
        // it
        ...
        mxArray*     data =
            mxCreateNumericArray(3, &dims[0], mxSINGLE_CLASS, mxREAL);
