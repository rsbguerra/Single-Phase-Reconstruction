...
namespace OpenEXRforMatlab {
    ...
    ///////////////////////////////////////////////////////////////////////////////
    template<typename T> struct mex_traits {
    ...
    template<typename T> inline mxArray* fromScalar(T n) {
        mxArray* arr =
            mxCreateNumericMatrix(1, 1, mex_traits<T>::classID, mxREAL);
    ...
    inline mxArray* fromArray(const T (&arr)[N]) {
        mxArray* mArr =
            mxCreateNumericMatrix(1, N, mex_traits<T>::classID, mxREAL);
        T* ptr = reinterpret_cast<T*>(mxGetPr(mArr));
        for (size_t i = 0; i != N; ++i) { ptr[i] = arr[i]; }
        return mArr;
    }
