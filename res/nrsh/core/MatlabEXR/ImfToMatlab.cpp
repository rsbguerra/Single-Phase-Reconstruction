...
namespace {
    ...
    template<typename T> inline bool canCastTo(const Attribute &a) {
        return dynamic_cast<const T*>(&a) != NULL;
    }

    // Utility to get the value of a typed attribute. It assumes
    // that canCastTo<T> returns true!
    template<typename T> inline const T &getValue(const Attribute &a) {
    ...
    inline mxArray* fromVector(const VecType<T> &vec) {
        ...
        mxArray*  mArr =
            mxCreateNumericMatrix(1, ndim, mex_traits<T>::classID, mxREAL);
        T* ptr = static_cast<T*>(mxGetData(mArr));
        for (int i = 0; i != ndim; ++i) { ptr[i] = vec[i]; }
        return mArr;
    }
    ...
    inline mxArray* fromMatrix(const MatrixType<T> &m) {
        ...
        mxArray* mArr =
            mxCreateNumericMatrix(ndim, ndim, mex_traits<T>::classID, mxREAL);
        T* ptr = static_cast<T*>(mxGetData(mArr));
        // Matlab uses column-major matrices
        for (int j = 0; j < ndim; ++j) {
            for (int i = 0; i < ndim; ++i) { *ptr++ = m[i][j]; }
        }
        return mArr;
    }
    ...
    inline mxArray* fromChannelList(const ChannelList &channelList) {
        ...
        const char* fields[] = {"name", "type", "perceptionlinear", "xsampling",
                                "ysampling"};
        const size_t nFields = sizeof(fields) / sizeof(const char*);
        mxArray*     cStruct =
            mxCreateStructMatrix(1, nChannels, nFields, &fields[0]);
...
mxArray* OpenEXRforMatlab::toMatlab(
    const OPENEXR_IMF_INTERNAL_NAMESPACE::Attribute &attr) {
