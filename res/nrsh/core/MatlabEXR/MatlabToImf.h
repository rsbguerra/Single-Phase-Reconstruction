...
namespace OpenEXRforMatlab {
    ...
    inline bool isVector(const mxArray* pa, mwSize &outNumel) {
        mwSize M, N;
        if (!isMatrix(pa, M, N)) { return false; }
    ...
    ///////////////////////////////////////////////////////////////////////////////
    // Conversion from Matlab to C++. Return true if the conversion is
    // successful
    ///////////////////////////////////////////////////////////////////////////////

    template<typename T> inline bool toNative(const mxArray* pa, T &outValue) {
        return false;
    }
    ...
    template<> inline bool toNative(const mxArray* pa, std::string &outValue) {
    ...
    inline bool toNative(const mxArray* pa, std::vector<std::string> &outVec) {
        ...
        if (mxIsChar(pa)) {
            std::string result;
            if (!toNative(pa, result)) { return false; }
        ...
        for (mwIndex i = 0; i != numel; ++i) {
            ...
            if (!toNative(element, result)) { return false; }
            outVec.push_back(result);
        }
        return true;
    }
    ...
    template<>
    inline bool toNative(const mxArray*                             pa,
                         OPENEXR_IMF_INTERNAL_NAMESPACE::PixelType &outType) {
        char* data = mxArrayToString(pa);
        if (data == NULL) { return false; }
    ...
    template<>
    inline bool
    toNative(const mxArray*                               pa,
             OPENEXR_IMF_INTERNAL_NAMESPACE::Compression &outCompression) {
        char* data = mxArrayToString(pa);
        if (data == NULL) { return false; }
    ...
    namespace convert_detail {
        ...
        template<typename TargetType> struct type_traits {
            template<typename T> static TargetType cast(const T &n) {
                return static_cast<TargetType>(n);
            }
        };

        template<> struct type_traits<half> {
            template<typename T> static half cast(const T &n) {
    ...
    // Convert an array of the given numeric type into another preallocated
    // array.
    template<typename SourceType, typename TargetType>
    inline void convertData(TargetType* dest, const mxArray* pa,
                            const size_t len) {
    ...
    inline void convertData(TargetType* dest, const mxArray* pa,
                            mxClassID srcType, const size_t len) {
        assert(srcType == mxGetClassID(pa));

        switch (srcType) {
            case mxDOUBLE_CLASS: convertData<real64_T>(dest, pa, len); break;
            case mxSINGLE_CLASS: convertData<real32_T>(dest, pa, len); break;
            case mxINT8_CLASS: convertData<int8_T>(dest, pa, len); break;
            case mxUINT8_CLASS: convertData<uint8_T>(dest, pa, len); break;
            case mxINT16_CLASS: convertData<int16_T>(dest, pa, len); break;
            case mxUINT16_CLASS: convertData<uint16_T>(dest, pa, len); break;
            case mxINT32_CLASS: convertData<int32_T>(dest, pa, len); break;
            case mxUINT32_CLASS: convertData<uint32_T>(dest, pa, len); break;
            case mxINT64_CLASS: convertData<int64_T>(dest, pa, len); break;
            case mxUINT64_CLASS: convertData<uint64_T>(dest, pa, len); break;

            default:
                assert("Unsupported mxClassID" == 0);
                mexErrMsgIdAndTxt("OpenEXR:unsupported",
                                  "Unsupported mxClassID: %s",
                                  mxGetClassName(pa));
        }
    }
